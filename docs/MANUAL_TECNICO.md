...
|---|---|---|
| Node.js | 20.x LTS | Runtime del servidor (desarrollo) |
| npm | 9.x | Gestor de paquetes |
| Docker Desktop | 4.x | Contenedores (produccion recomendada) |
| PostgreSQL | 16 | Base de datos (si no usa Docker) |
...
| Puerto | Servicio | Uso |
|---|---|---|
| 80 | Cliente HTTP | Interfaz web |
| 443 | Cliente HTTPS | SSL |
...
- Administracion basica de Docker (docker-compose)
...
### 3.2 Diagrama de Contenedores (Docker)
...
|                                                                     |
|  +------------------+     +------------------+     +-------------+  |
|  |   nginx (Client) |---->| Express (Server) |---->| PostgreSQL  |  |
|  |   :80 / :443     |     | :3000            |     | :5432       |  |
|  |   SPA + Proxy    |     | API REST         |     | Datos+Fotos |  |
|  +------------------+     +------------------+     +-------------+  |
|        ^                                                        ^   |
|        |   (bind mounts)                                       |   |
|  +-----+-----+                                          +------+   |
...
```
1. Registro: Guardia busca visitante por cedula o crea nuevo
2. Captura: Foto del rostro + foto del documento de identidad
3. Check-in: Guardia completa datos y confirma check-in
4. Estado "waiting": Visita en espera de ser admitida
5. Admitir: Guardia o Admin admite al visitante (-> "active")
...
├── role (ENUM: admin|guard|auditor|superadmin)
...
IntermittentLogs / VisitIntervals
├── id (PK, INTEGER, autoIncrement)
...
| 005 | `005-add-photo-blobs-and-intermittent.sql` | photo_data, id_photo_data en Visitors; VisitIntervals |
...
### 4.4 SuperAdmin (Gestion de Usuarios)

```
GET    /api/v1/superadmin/users                    # Listar usuarios
POST   /api/v1/superadmin/users                    # Crear usuario
PUT    /api/v1/superadmin/users/:id                # Actualizar usuario
DELETE /api/v1/superadmin/users/:id                # Eliminar usuario
POST   /api/v1/superadmin/users/:id/reset-password # Resetear contrasena
GET    /api/v1/superadmin/audit-logs               # Logs de auditoria
```
...
| EmailService | `infrastructure/services/EmailService.ts` | Envio de correos (nodemailer) |
...
Jobs:
1. **server-tests**: PostgreSQL service container → `vitest run` en server/
...
### 12.1 Docker Compose (Produccion)

```bash
scripts\deploy.bat  # Script completo: auto-env + build + up
```
...
| Script | Proposito |
|---|---|
| `start.bat` | Inicio inteligente (detecta primera vez o reinicio) |
| `detener.bat` | `docker-compose down` |
| `status.bat` | Estado de contenedores + URLs de acceso |
| `verify-system.bat` | Verificacion completa de salud del sistema |
| `auto-env.bat` | Detecta IP LAN y configura .env + CORS |
| `setup-ssl.bat` | Genera certificados SSL autofirmados |
| `monitor-health.bat` | Monitoreo continuo en tiempo real |
| `deploy.bat` | Despliegue completo automatizado |
