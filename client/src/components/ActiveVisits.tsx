    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {visits.map((visit) => {
                ...
                const timeInSite = getTimeInSite(visit.check_in);

                return (
                    <div
                        key={visit.id}
                        className="group panel-tech rounded-2xl transition-all duration-300 overflow-visible border border-[color:var(--border-1)] flex flex-col relative transform hover:-translate-y-1"
                    >
                        ...
                        <div className="px-5 py-3 border-t border-[color:var(--border-1)] bg-[color:var(--surface-2)]/60 flex justify-between items-center mt-auto">
                            ...
                            <button
                                onClick={() => handleCheckout(visit.id, visitorName)}
                                disabled={isCheckingOut}
                                className="group/btn relative px-4 py-1.5 rounded-lg border border-[color:var(--border-1)] text-[color:var(--text-2)] text-xs font-semibold hover:border-red-400 hover:text-red-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 overflow-hidden"
                            >
