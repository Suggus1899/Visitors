...
export class UpdateArcoRequestStatusUseCase {
  ...
  async execute(id: number, dto: UpdateArcoRequestStatusDto, actorId: number, actorUsername: string, ip?: string, userAgent?: string): Promise<{ id: number; status: ArcoRequestStatus; resolvedAt: Date | null; resolutionNotes: string | null }> {
    const existing = await this.arcoRepository.findById(id);
    if (!existing) {
      throw new Error('NOT_FOUND');
    }

    const terminalStatus = dto.status === 'completed' || dto.status === 'rejected';
    const updated = await this.arcoRepository.update(id, {
      status: dto.status,
