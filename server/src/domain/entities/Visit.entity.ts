...
export class Visit {
   ...
   * Business rule: checkout is only allowed from ACTIVE status
   */
  checkout(checkOutTime: Date, notes?: string): Visit {
    if (this.status === VisitStatus.INTERMITTENT) {
      throw new Error('No se puede hacer checkout desde estado Intermitente. Debe regresar a estado Activo primero.');
    }
